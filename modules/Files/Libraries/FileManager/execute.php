    ...
    switch ($_GET['action']) {
            ...
            if ($rename_folders) {
                ...
                if (!empty($name)) {
                    if (!rename_folder($path, $name, $transliteration, $convert_spaces)) {
                        response(file_manager_trans('Rename_existing_folder'), 403)->send();
                        exit;
                    }
                    ...
                    response(file_manager_trans('Empty_name'), 400)->send();
                    exit;
                }
            }
            break;
        case 'create_file':
            if ($create_text_files === false) {
                response(sprintf(file_manager_trans('File_Open_Edit_Not_Allowed'), strtolower(file_manager_trans('Edit'))), 403)->send();
                exit;
            }
            ...
            if (strpos($name, '.') === false) {
                response(file_manager_trans('No_Extension') . ' ' . sprintf(file_manager_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
                exit;
            }
            ...
            if (!in_array(end($parts), $editable_text_file_exts)) {
                response(file_manager_trans('Error_extension') . ' ' . sprintf(file_manager_trans('Valid_Extensions'), implode(', ', $editable_text_file_exts)), 400)->send();
                exit;
            }

            // file already exists
            if (file_exists($path . $name)) {
                response(file_manager_trans('Rename_existing_file'), 403)->send();
                exit;
            }

            $content = $_POST['new_content'];
            if (@file_put_contents($path . $name, $content) === false) {
                response(file_manager_trans('File_Save_Error'), 500)->send();
                exit;
            } else {
                if (is_function_callable('chmod') !== false) {
                    chmod($path . $name, 0644);
                }
                response(file_manager_trans('File_Save_OK'))->send();
                exit;
            }
...
//                response()->make('Can\'t paste same folder existed')->send();
                response()->make('Can\'t paste same folder or file existed')->send();
                exit;
            }

            // user wants to paste folder to it's own sub folder.. baaaah.
            if (is_dir($data['path']) && strpos($path, $data['path']) !== false) {
                response()->make('Can\'t paste in own folder')->send();
                exit;
            }

            // something terribly gone wrong
            if ($action != 'copy' && $action != 'cut') {
                response()->make('no action', 400)->send();
                exit;
            }

            // check for writability
            if (is_really_writable($path) === false || is_really_writable($path_thumb) === false) {
                response(file_manager_trans('Dir_No_Write') . '<br/>' . str_replace('../', '', $path) . '<br/>' . str_replace('../', '', $path_thumb), 403)->send();
                exit;
            }

            // check if server disables copy or rename
            if (is_function_callable(($action == 'copy' ? 'copy' : 'rename')) === false) {
                response(sprintf(file_manager_trans('Function_Disabled'), ($action == 'copy' ? lcfirst(file_manager_trans('Copy')) : lcfirst(file_manager_trans('Cut')))), 403)->send();
                exit;
            }
        ...
        case 'chmod':
            ...
            // check perm
            if ($chmod_perm === false) {
                response(sprintf(file_manager_trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(file_manager_trans('Folders')) : lcfirst(file_manager_trans('Files')))), 403)->send();
                exit;
            }

            // check mode
            if (!preg_match("/^[0-7]{3}$/", $mode)) {
                response(file_manager_trans('File_Permission_Wrong_Mode'), 400)->send();
                exit;
            }
            ...
            if (is_function_callable('chmod') === false) {
                response(sprintf(trafile_manager_transns('Function_Disabled'), 'chmod'), 403)->send();
                exit;
            }
