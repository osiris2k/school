...
$lib_path = config('files.module_path') . config('files.repo_path') . '/FileManager';
    ...
    switch ($_GET['action']) {
        ...
        case 'sort':
            ...
            if (isset($_GET['descending'])) {
                $_SESSION['RF']["descending"] = $_GET['descending'] == 1;
            }
            break;
        ...
        case 'save_img':
            ...
            if ($image_data === false) {
                response(file_manager_trans('Aviary_No_Save'), 400)->send();
                exit;
            }
...
//                                    dd('zip://' . $path . '#' . $OnlyFileName, $base_folder . $FullFileName['name']);
                                    copy('zip://' . public_path($path) . '#' . $OnlyFileName, $base_folder . $FullFileName['name']);
                                }
                            }
                        }
                        $zip->close();
                    } else {
                        response(file_manager_trans('Zip_No_Extract'), 500)->send();
                        exit;
                    }
                ...
                default:
                    response(file_manager_trans('Zip_Invalid'), 400)->send();
                    exit;
            }
            break;
        case 'media_preview':
            ...
            // Remove duplicate upload folder string
            $countDup = substr_count($preview_file, 'uploads/');
            $preview_file = ($countDup > 1) ? asset(substr($preview_file, 8)) : asset($preview_file);
        ...
        case 'copy_cut':
            ...
            if (is_dir($path)) {
                ...
                if ($copy_cut_dirs === false) {
                    response(sprintf(file_manager_trans('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(file_manager_trans('Copy')) : lcfirst(file_manager_trans('Cut'))), file_manager_trans('Folders')), 403)->send();
                    exit;
                }
                ...
                if ($copy_cut_max_size !== false && is_int($copy_cut_max_size)) {
                    ...
                    if (($copy_cut_max_size * 1024 * 1024) < $sizeFolderToCopy) {
                        response(sprintf(file_manager_trans('Copy_Cut_Size_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(file_manager_trans('Copy')) : lcfirst(file_manager_trans('Cut'))), $copy_cut_max_size), 400)->send();
                        exit;
                    }
                }
                ...
                if ($copy_cut_max_count !== false && is_int($copy_cut_max_count)) {
                    if ($copy_cut_max_count < filescount($path)) {
                        response(sprintf(file_manager_trans('Copy_Cut_Count_Limit'), ($_POST['sub_action'] == 'copy' ? lcfirst(file_manager_trans('Copy')) : lcfirst(file_manager_trans('Cut'))), $copy_cut_max_count), 400)->send();
                        exit;
                    }
                }
            } else {
                ...
                if ($copy_cut_files === false) {
                    response(sprintf(file_manager_trans('Copy_Cut_Not_Allowed'), ($_POST['sub_action'] == 'copy' ? lcfirst(file_manager_trans('Copy')) : lcfirst(file_manager_trans('Cut'))), file_manager_trans('Files')), 403)->send();
                    exit;
                }
            }
        ...
        case 'chmod':
            ...
            if (
                ...
                response(sprintf(file_manager_trans('File_Permission_Not_Allowed'), (is_dir($path) ? lcfirst(file_manager_trans('Folders')) : lcfirst(file_manager_trans('Files'))), 403), 400)->send();
                exit;
            } else {
				...
				<form id="chmod_form">
					<table class="table file-perms-table">
						...
						<tbody>
							...
							<tr>
								<td>' . file_manager_trans('Group') . '</td>
                ...
                if (is_dir($path)) {
                    $ret .= '<div class="hero-unit" style="padding:10px;">' . file_manager_trans('File_Permission_Recursive') . '<br/><br/>
							<ul class="unstyled">
								<li><label class="radio"><input value="none" name="apply_recursive" type="radio" checked> ' . file_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_trans('No') . '</label></li>
								<li><label class="radio"><input value="files" name="apply_recursive" type="radio"> ' . file_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_trans('Files') . '</label></li>
								<li><label class="radio"><input value="folders" name="apply_recursive" type="radio"> ' . file_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_trans('Folders') . '</label></li>
								<li><label class="radio"><input value="both" name="apply_recursive" type="radio"> ' . file_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_transfile_manager_trans('Files') . ' & ' . file_manager_trans('Folders') . '</label></li>
							</ul>
							</div>';
                }
            ...
            break;
        case 'get_lang':
            if (!file_exists($lib_path . '/lang/languages.php')) {
                echo 1;
                response(file_manager_trans('Lang_Not_Found'), 404)->send();
                exit;
            }

            $languages = include $lib_path . '/lang/languages.php';
        ...
        case 'change_lang':
            ...
            if (!file_exists($lib_path . '/lang/' . $choosen_lang . '.php')) {
                response(file_manager_trans('Lang_Not_Found'), 404)->send();
                exit;
            }

            $_SESSION['RF']['language'] = $choosen_lang;
            $_SESSION['RF']['language_file'] = $lib_path . 'lang/' . $choosen_lang . '.php';
        ...
        case 'get_file': // preview or edit
            ...
            // Remove duplicate upload folder string
            $countDup = substr_count($selected_file, 'uploads/');
            $selected_file = ($countDup > 1) ? substr($selected_file, 8) : $selected_file;

            if (!file_exists($selected_file)) {
                response(file_manager_trans('File_Not_Found'), 404)->send();
                exit;
            }
            ...
            if (!in_array($info['extension'], $allowed_file_exts)
                ...
                response(sprintf(file_manager_trans('File_Open_Edit_Not_Allowed'), ($sub_action == 'preview' ? strtolower(file_manager_trans('Open')) : strtolower(file_manager_trans('Edit')))), 403)->send();
                exit;
            }
            echo $sub_action;
            echo $preview_mode;
            if ($sub_action == 'preview') {
                if ($preview_mode == 'text') {
                    ...
                    $ret = '<iframe id="viewer" src="/' . $_GET["file"] . '" allowfullscreen="" webkitallowfullscreen="" class="viewer-iframe"></iframe>';
