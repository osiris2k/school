...
/*
 * Change function name from "trans" to "file_manager_trans"
 * because its duplicate Laravel function "trans"
 */
if (!function_exists('file_manager_trans')) {
     ...
     */
    function file_manager_trans($var)
    {
        global $lang_vars;
...
function create_img($imgfile, $imgthumb, $newwidth, $newheight = null, $option = "crop")
{
    ...
    if ((file_exists($imgfile) || strpos($imgfile, 'http') === 0) &&
        (strtolower(pathinfo($imgfile, PATHINFO_EXTENSION)) != 'svg')
    ) {
...
{
    $countDup = substr_count($source, 'thumbs/uploads/');
    $source = ($countDup > 0) ? str_replace('uploads/', '', $source) : $source;
