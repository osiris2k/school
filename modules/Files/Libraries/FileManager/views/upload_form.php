    ...
    <div class="tabbable upload-tabbable"> <!-- Only required for left/right tabs -->
        ...
        <div class="tab-content">
            <div class="tab-pane active" id="tab1">
                <?php } ?>
                <?php
                $uploadPathCon = (isset($_GET['type'])) ? $_GET['type'] : '';
                switch ($uploadPathCon) {
                    case'1':
                        $uploadPath = config('upload.IMAGES_UPLOAD_DIR') . '/';
                        break;
                    case'2':
                        $uploadPath = config('upload.FILE_ROOT_DIR') . '/';
                        break;
                    /*case'3':
                        $uploadPath = config('upload.IMAGES_UPLOAD_DIR') . '/';
                        break;*/
                ...
                ?>
                <form action="<?php echo route('files.system.post_index'); ?>" method="post"
                      enctype="multipart/form-data"
                      id="rfmDropzone" class="dropzone">
                    <input type="hidden" id="_token" name="_token" value="<?php echo csrf_token(); ?>"/>
                    <input type="hidden" name="path" value="<?php echo $cur_path ?>"/>
                    <!--<input type="hidden" name="path" value="<?php echo $uploadPath ?>"/>-->
