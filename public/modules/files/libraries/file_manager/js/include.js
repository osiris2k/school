/**
 * Notes: edit script details
 *   - add token variable and pass token value to its.
 *   - add token variable to all ajax request
 *   - add executePath variable for alternative way to execute execute.php
 *   - add ajaxCallPath variable for alternative way to execute ajax_call.php
 **/
var encodeURL, show_animation, hide_animation, apply, apply_none, apply_img, apply_any, apply_video, apply_link, apply_file_rename, apply_file_duplicate, apply_folder_rename, token, executePath, ajaxCallPath;
...
!function (e, a, t) {
            ...
            callback: function () {
                ...
                if (null !== a) {
                    ...
                    e.ajax({
                        ...
                        url: executePath + "?action=create_file",
                        data: {
                            path: n,
                            name: a,
                            new_content: t,
    ...
    function r(a) {
        ...
        e.ajax({
            ...
            url: ajaxCallPath.post_method + "?action=get_file&sub_action=edit&preview_mode=text",
            data: {
                path: t,
                //_token: token
            }
            ...
            bootbox.dialog(n, [{
                ...
                callback: function () {
                    ...
                    e.ajax({
                        ...
                        url: executePath + "?action=save_text_file",
                        data: {
                            path: t,
                            new_content: a,
                            //_token: token
    ...
    function o() {
        e.ajax({
            ...
            url: ajaxCallPath.post_method + "?action=get_lang",
            //data: {_token: token}
        }).done(function (a) {
            bootbox.dialog(a, [{
                label: e("#cancel").val(),
                ...
                callback: function () {
                    ...
                    e.ajax({
                        ...
                        url: ajaxCallPath.post_method + "?action=change_lang",
                        data: {
                            choosen_lang: a,
                            //_token: token
                        }
    ...
    function l(a) {
        ...
        t = a.find(".rename-file-paths").attr(a.hasClass("directory") ? "data-path" : "data-path"), e.ajax({
            ...
            url: ajaxCallPath.post_method + "?action=chmod",
            data: {
                path: t,
                //_token: token
            }
    ...
    function d() {
        bootbox.confirm(e("#lang_clear_clipboard_confirm").val(), e("#cancel").val(), e("#ok").val(), function (a) {
            1 == a && e.ajax({
                ...
                url: ajaxCallPath.post_method + "?action=clear_clipboard",
                //data: {_token: token}
            }).done(function (a) {
    ...
    function p(a, t) {
        if ("copy" == t || "cut" == t) {
            ...
            n = a.find(".rename-file-paths").attr(a.hasClass("directory") ? "data-path" : "data-path"), e.ajax({
                ...
                url: ajaxCallPath.post_method + "?action=copy_cut",
                data: {
                    path: n,
                    sub_action: t,
                    //_token: token
        ...
        bootbox.confirm(e("#lang_paste_confirm").val(), e("#cancel").val(), e("#ok").val(), function (t) {
            if (1 == t) {
                ...
                n = "undefined" != typeof a ? a.find(".rename-folder").attr("data-path") : e("#sub_folder").val() + e("#fldr_value").val(), e.ajax({
                    ...
                    url: executePath + "?action=paste_clipboard",
                    data: {
                        path: n,
                        //_token: token
                    }
    ...
    function g() {
        /**
         * Add multiple file condition for handler popup close.
         */
        //if (e('#multiple_file').val() != 'TRUE') {
    ...
    var j = "9.9.6",
        ...
        U = {
            contextActions: {
                ...
                unzip: function (a) {
                    ...
                    e.ajax({
                        ...
                        url: ajaxCallPath.post_method + "?action=extract",
                        data: {
                            path: t,
                            //_token: token
                        }
            ...
            bindGridEvents: function () {
                // edit this function to multiple select file
                function a(a) {
                ...
                e("#filter-input").on("keyup", function () {
                    ...
                    e(this).val(t), a && O(function () {
                        e("li", "ul.grid ").each(function () {
                            ...
                            url: ajaxCallPath.get_method + "?action=filter&type=" + t
                        }).done(function (e) {
                            "" != e && bootbox.alert(e)
                        }), O(function () {
            ...
            makeSort: function (a) {
                ...
                e(".sorter").on("click", function () {
                    ...
                    t = e("#sort_by").val() === n.attr("data-sort") ? 0 == t ? !0 : !1 : !0, a ? (e.ajax({
                        ...
                        url: ajaxCallPath.get_method + "?action=sort&sort_by=" + n.attr("data-sort") + "&descending=" + (t ? 1 : 0)
    ...
    e(document).ready(function () {
        ...
        if (U.makeSort(t), U.makeFilters(t), e("#info").on("click", function () {
                ...
                bootbox.prompt(e("#insert_folder_name").val(), e("#cancel").val(), e("#ok").val(), function (a) {
                    if (null !== a) {
                        ...
                        e.ajax({
                            ...
                            url: executePath + "?action=create_folder",
                            data: {
                                path: t,
                                name: a,
                                //_token: token
                ...
                e(".view-controller button").removeClass("btn-inverse"), e(".view-controller i").removeClass("icon-white"), a.addClass("btn-inverse"), a.find("i").addClass("icon-white"), e.ajax({
                    ...
                    url: ajaxCallPath.get_method + "?action=view&type=" + a.attr("data-value")
                }).done(function (e) {
                    "" != e && bootbox.alert(e)
        ...
        if ("" != t)
            if (1 == e("#crossdomain").val()) n.postMessage({
                ...
                /**
                 * Override default single select file
                 */
                if (e('#multiple_file').val() == 'TRUE') {
                    storeMultipleFile({
                        cur_dir: i,
        ...
        // Fix File bug after rename
        var aPre = a.find("a.file-preview-btn");
        if (aPre.attr('data-url')) {
            var aPreAttr = aPre.attr('data-url'),
                aPreRep = aPreAttr.replace(r, t + "." + o);
            aPre.attr('data-url', aPreRep);
        }
        ...
        var i = a.find(".folder-link"),
            r = i.attr("href"),
            o = e("#fldr_value").val(),
        // Replace old folder url by new folder url (new folder name)
            l = r.replace("fldr=" + encodeURIComponent(o) + encodeURIComponent(n), "fldr=" + encodeURIComponent(o) + encodeURIComponent(t));
        i.each(function () {
            e(this).attr("href", l)
        });
    ...
    /**
     * Store selected file for multiple select purpose
     * @param object selectedData:{cur_dir,sub_dir,base_url,selected_file}
     */
    function storeMultipleFile(selectedData) {
        var storeEle = $('#multiple_file_value');
        var storeVal = [];
        var indexRemove = 0;
        var figureContainFile = $('figure[data-name="' + selectedData.selected_file + '"]');
        var selectedImgEle = figureContainFile.children('a').children('div.img-precontainer').children('div.img-container').children('img');


        if (storeEle.val().length != 0) {
            storeVal = JSON.parse(storeEle.val());
        }

        indexRemove = storeVal.indexOf(selectedData.cur_dir + selectedData.selected_file);

        if (indexRemove !== undefined && indexRemove != -1) {
            storeVal.splice(indexRemove, 1);
            storeEle.val(JSON.stringify(storeVal));
            $(selectedImgEle).attr('style', 'border:none;');
        } else {

            if (e('#multiple_file').val() != 'TRUE') {
                console.log(storeVal,storeVal.length);
                if (storeVal.length == 1) {
                    bootbox.alert("You can select a maximum one file.");
                    return false;
                }
            }

            storeVal.push(selectedData.cur_dir + selectedData.selected_file);
            storeEle.val(JSON.stringify(storeVal));
            $(selectedImgEle).attr('style', 'border:5px outset #0088cc;');
        }

    }

    /**
     * Two function below not use now.
     * Its use for user can't select duplicate file
     */
    // TODO store selected file in session for prevent value missing after redirect page
    function storeSelectedFileInSession() {

    }

    // TODO call from window parent to sync file again
    function syncSelectedData() {
        console.log('sync file in iframe call from parent');
    }
