...
@section('content')
    @if(count($sites) > 0)
        ...
        <div class="col-xs-3">
            <label class="control-label col-xs-2">Sites:</label>
            <div class="col-xs-10">
...
@section('script')
    ...
    <script type="text/javascript">
        $('#reorderFormFilterBt').click(function () {
            var contentObjTypeId = '{{$content_object_type->id}}';
            var siteId = $('#site-filter').val();
            var contentId = $('#content-filter').val();
            var redirectUrl = '{{route('reorder.system.get_get_reorder_list')}}/' + $('#site-filter').find('option:selected').attr('data-type') + '/' + contentObjTypeId + '/' + siteId + '/' + contentId;
        ...
        $(document).ready(function () {
            ...
            $("#site-filter").change(function (e) {
                var contentObjTypeId = '{{$content_object_type->id}}';
                var siteId = $('#site-filter').val();
                var redirectUrl = '{{route('reorder.system.get_get_reorder_list')}}/' + $('#site-filter').find('option:selected').attr('data-type') + '/' + contentObjTypeId + '/' + siteId + '/0';
                window.location.href = redirectUrl;
            });
        });
    </script>
